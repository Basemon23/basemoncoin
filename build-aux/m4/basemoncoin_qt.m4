...
dnl Output: If qt version is auto, set basemoncoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$basemoncoin_qt_want_version" = xauto && test "x$basemoncoin_qt_force" != xyes; then
    if test "x$basemoncoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; basemoncoin-qt frontend will not be built])
    fi
    basemoncoin_enable_qt=no
    basemoncoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$basemoncoin_enable_qt" != xno && test "x$basemoncoin_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build basemoncoin-qt GUI (default=auto)])],
    [
     basemoncoin_qt_want_version=$withval
     if test "x$basemoncoin_qt_want_version" = xyes; then
       basemoncoin_qt_force=yes
       basemoncoin_qt_want_version=auto
     fi
    ],
    [basemoncoin_qt_want_version=auto])
...
dnl Outputs: basemoncoin_enable_qt, basemoncoin_enable_qt_dbus, basemoncoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$basemoncoin_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    basemoncoin_enable_qt=yes
    basemoncoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      basemoncoin_enable_qt_test=no
    fi
    basemoncoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      basemoncoin_enable_qt_dbus=yes
    fi
  ...
  if test x$basemoncoin_enable_qt = xyes; then
    AC_MSG_RESULT([$basemoncoin_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$basemoncoin_enable_qt])
  fi
...
dnl Output: basemoncoin_cv_qt5=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, basemoncoin_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [basemoncoin_cv_qt5=yes],
    [basemoncoin_cv_qt5=no])
])])

dnl Internal. Check if the included version of Qt is greater than Qt58.
dnl Requires: INCLUDES must be populated as necessary.
dnl Output: basemoncoin_cv_qt58=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, basemoncoin_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [basemoncoin_cv_qt58=yes],
    [basemoncoin_cv_qt58=no])
])])
...
dnl Output: basemoncoin_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, basemoncoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
      ...
      [basemoncoin_cv_static_qt=yes],
      [basemoncoin_cv_static_qt=no])
    ])
  if test "x$basemoncoin_cv_static_qt" = xyes; then
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$basemoncoin_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, basemoncoin_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [basemoncoin_cv_need_platformsupport=yes],
           [basemoncoin_cv_need_platformsupport=no])
         ])
         if test "x$basemoncoin_cv_need_platformsupport" = xyes; then
           if test x$basemoncoin_cv_qt58 = xno; then
...
dnl Inputs: basemoncoin_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If basemoncoin_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: basemoncoin_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$basemoncoin_qt_want_version" = xauto; then
