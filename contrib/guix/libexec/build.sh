...
# Determine output paths to use in BASES_* environment variables
BASES_GLIBC="$(store_path glibc-cross-${HOST})"
BASES_GLIBC_STATIC="$(store_path glibc-cross-${HOST} static)"
BASES_KERNEL="$(store_path linux-libre-headers-cross-${HOST})"
BASES_GCC="$(store_path gcc-cross-${HOST})"
BASES_GCC_LIBS=( "${BASES_GCC}/lib/gcc/${HOST}"/* ) # This expands to an array of directories...
BASES_GCC_LIB="${BASES_GCC_LIBS[0]}" # ...we just want the first one (there should only be one)

# Set environment variables to point Guix's cross-toolchain to the right
# includes/libs for $HOST
#
# NOTE: BASES_C_INCLUDE_PATH is missing ${BASES_GCC_LIB}/include-fixed, because
# the limits.h in it is missing a '#include_next <limits.h>'
#
export BASES_C_INCLUDE_PATH="${BASES_GCC_LIB}/include:${BASES_GLIBC}/include:${BASES_KERNEL}/include"
export BASES_CPLUS_INCLUDE_PATH="${BASES_GCC}/include/c++:${BASES_GCC}/include/c++/${HOST}:${BASES_GCC}/include/c++/backward:${BASES_C_INCLUDE_PATH}"
export BASES_LIBRARY_PATH="${BASES_GCC}/lib:${BASES_GCC}/${HOST}/lib:${BASES_GCC_LIB}:${BASES_GLIBC}/lib:${BASES_GLIBC_STATIC}/lib"

# Sanity check BASES_*_PATH directories
IFS=':' read -ra PATHS <<< "${BASES_C_INCLUDE_PATH}:${BASES_CPLUS_INCLUDE_PATH}:${BASES_LIBRARY_PATH}"
for p in "${PATHS[@]}"; do
    if [ ! -d "$p" ]; then
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform basemoncoin-linux-g++'
...
if [ -z "$(find "${OUTDIR}/src" -name 'basemoncoin-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'basemoncoin-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'basemoncoin-*.tar.gz')"
# Determine our distribution name (e.g. basemoncoin-0.18.0)
