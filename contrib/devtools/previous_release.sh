  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/Basemon23/basemoncoin "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/basemoncoind src/basemoncoin-cli src/basemoncoin-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/basemoncoin-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/basemoncoin-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="bin/basemoncoin-core-${tag:1}"
        fi
        URL="https://basemoncoin.org/$BIN_PATH/basemoncoin-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        if ! curl -O -f $URL; then
            echo "Download failed."
            exit 1
        fi
        tar -zxf "basemoncoin-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "basemoncoin-${tag:1}"
        rm "basemoncoin-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
