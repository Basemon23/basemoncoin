...
###   basemoncoincore.org and basemoncoin.org and compares them.
...
WORKINGDIR="/tmp/basemoncoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://basemoncoincore.org"
HOST2="https://basemoncoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="basemoncoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'basemoncoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "basemoncoin.org failed to provide signature file, but basemoncoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "basemoncoin.org and basemoncoincore.org signature files were not equal?"
