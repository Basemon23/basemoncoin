...
#include <qt/basemoncoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Basemoncoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no basemoncoin: URI
    if(!uri.isValid() || uri.scheme() != QString("basemoncoin"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("Basemoncoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openBitcoinConf()
{
    ...
    /* Open basemoncoin.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "basemoncoin.desktop";
    return GetAutostartDir() / strprintf("basemoncoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a basemoncoin.desktop file to the autostart directory:
