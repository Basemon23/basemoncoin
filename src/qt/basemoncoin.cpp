...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. basemoncoin_de.qm (shortcut "de" needs to be defined in basemoncoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. basemoncoin_de_DE.qm (shortcut "de_DE" needs to be defined in basemoncoin.qrc)
...
static const char* qt_argv = "basemoncoin-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // basemoncoin: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(basemoncoin);
    Q_INIT_RESOURCE(basemoncoin_locale);
    ...
    /// 6. Determine availability of data directory and parse basemoncoin.conf
    ...
    // basemoncoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
