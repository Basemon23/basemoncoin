...
const char * const BITCOIN_CONF_FILENAME = "basemoncoin.conf";
 ...
 * https://github.com/Basemon23/basemoncoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //basemoncoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "basemoncoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Basemoncoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Basemoncoin
    // Mac: ~/Library/Application Support/Basemoncoin
    // Unix: ~/.basemoncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Basemoncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Basemoncoin";
#else
    // Unix
    return pathRet / ".basemoncoin";
#endif
#endif
}
