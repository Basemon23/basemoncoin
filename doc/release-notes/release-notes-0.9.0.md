Bitcoin Core version 0.9.0 is now available from:
  ...
  https://basemoncoin.org/bin/0.9.0/
...
over /Applications/Bitcoin-Qt (on Mac) or basemoncoind/basemoncoin-qt (on Linux).
...
TX outputs, bloating basemoncoin's UTXO database.
...
basemoncoind makes it easier for experienced open source Developers to contribute 
to the project.
...
Another change in the 0.9 release is moving away from the basemoncoind executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running basemoncoin daemon to do THIS") was split into a separate
executable, 'basemoncoin-cli'. The RPC client code will eventually be removed from
basemoncoind, but will be kept for backwards compatibility for a release or two.
...
- Add '-disablewallet' mode to let basemoncoind run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with basemoncoind
...
- Added new DNS seed from basemoncoinstats.com
...
- Show and store message of normal basemoncoin:URI
- Fix richtext detection hang issue on very old Qt versions
- OS X: Make use of the 10.8+ user notification center to display Growl-like 
  notifications
- OS X: Added NSHighResolutionCapable flag to Info.plist for better font
  rendering on Retina displays.
- OS X: Fix basemoncoin-qt startup crash when clicking dock icon
- Linux: Fix Gnome basemoncoin: URI handler
...
- Add separate basemoncoin-cli client
