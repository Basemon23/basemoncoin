Bitcoin Core version 0.17.0 is now available from:
  ...
  <https://basemoncoincore.org/bin/basemoncoin-core-0.17.0/>
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `basemoncoin.conf` file, not inside included files or from
  ...
  as basemoncoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the basemoncoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=basemoncoin
    test.uacomment=basemoncoin-testnet
...
be used in V0.17 by starting basemoncoind with the '-deprecatedrpc=accounts'
...
[BIP 174 PSBT](https://github.com/basemoncoin-project/bips/blob/master/bip-0174.mediawiki) is an interchange format for Bitcoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://basemoncointalk.org/?topic=279249) transactions.
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When basemoncoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If basemoncoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart basemoncoind with
...
- When running basemoncoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If basemoncoind
...
- #11962 `0a01843` add seed.basemoncoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI basemoncoin:// (krab)
...
- #13486 `66e1a08` Move rpc/util.cpp from libbasemoncoin-util to libbasemoncoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for basemoncoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling basemoncoin-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to basemoncoin.conf (MarcoFalke)
...
- #12882 `d96bdd7` Make `test_basemoncoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.basemoncoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_basemoncoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in basemoncoind systemd service file (Flowdalic)
- #12652 `55f490a` basemoncoin-cli: Provide a better error message when basemoncoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to basemoncoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for basemoncoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` basemoncoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix basemoncoin-cli --version (Empact)
...
- #12877 `2b54155` Use basemoncoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update basemoncoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust basemoncoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed basemoncoin-cli -help output for help2man (hebasto)
