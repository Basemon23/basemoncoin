Bitcoin Core version 0.12.0 is now available from:
  ...
  <https://basemoncoin.org/bin/basemoncoin-core-0.12.0/>
...
[libsecp256k1](https://github.com/basemoncoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/basemoncoin-project/bips/blob/master/bip-0130.mediawiki)
...
(https://github.com/basemoncoin-project/bips/blob/master/bip-0125.mediawiki).
...
This allows running basemoncoind without having to do any manual configuration.
...
`basemoncoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/Basemon23/basemoncoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/basemoncoin-project/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/basemoncoin-project/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /basemoncoinrpc>
        ...
        # optional bypass basemoncoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< basemoncoinrpc:<password>
    </Location>
...
Restarting basemoncoind will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new basemoncoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include basemoncoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix basemoncoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of basemoncoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [basemoncoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libbasemoncoinconsensus (Braydon Fuller)
